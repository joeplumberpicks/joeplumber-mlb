...
def infer_pitcher_er_from_paths(
    season: int,
    features_path: Path,
    model_dir: Path,
    output_path: Path,
    *,
    ...
    out["er_pred"] = _predict(art["model"], art.get("model_type", "unknown"), features[feature_columns])
    out = out.sort_values(["game_date", "game_pk", "pitcher_id"], kind="mergesort").reset_index(drop=True)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    out.to_csv(output_path, index=False)
    log.info("wrote_pitcher_er_preds season=%s rows=%d path=%s", season, len(out), output_path)
    return out
