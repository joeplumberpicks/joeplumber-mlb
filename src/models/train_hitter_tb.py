...
def _resolve_target_col(targets_df: pd.DataFrame) -> str:
    for c in TARGET_CANDIDATES:
        if c in targets_df.columns:
            return c
    raise ValueError(f"targets missing Hitter TB target column; expected one of {TARGET_CANDIDATES}")
...
def run_smoke_test(logger: logging.Logger | None = None) -> None:
    ...
    y = 1.6 + 0.6 * np.nan_to_num(X[:, 0], nan=0.0) + rng.normal(0, 0.2, 40)
    preds = NumpyRidgeRegression(alpha=1.0).fit(X, y).predict(X)
    assert np.isfinite(preds).all()
    log.info("Hitter TB smoke test passed")


def train_hitter_tb_model(
    season: int,
    ...
    *,
    ...
    eval_path = Path("data/outputs") / f"hitter_tb_eval_{season}.csv"
    ...
    log.info("Hitter TB metrics: rmse=%.4f mae=%.4f r2=%.4f", rmse, mae, r2)
    log.info("Hitter TB reliability deciles:\n%s", rel.to_string(index=False))
...
def train_hitter_tb_from_paths(
    season: int,
    features_path: Path,
    targets_path: Path,
    model_dir: Path,
