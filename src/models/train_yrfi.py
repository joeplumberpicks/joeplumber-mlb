...
def run_smoke_test(logger: logging.Logger | None = None) -> None:
    ...
    X[rng.integers(0, 40, size=5), rng.integers(0, 5, size=5)] = np.nan
    y = (X[:, 0] + 0.3 * np.nan_to_num(X[:, 1], nan=0.0) > 0).astype(int)
    ...
    log.info("YRFI smoke test passed")
...
def train_yrfi_model(
    ...
    *,
    ...
    """Train YRFI model + calibrator and emit eval predictions for validation split."""
    log = logger or logging.getLogger(__name__)

    req_feat = {"game_date", "game_pk"}
    req_tgt = {"game_date", "game_pk", "yrfi"}
    ...
    targs["yrfi"] = pd.to_numeric(targs["yrfi"], errors="coerce").astype("Int64")

    merged = feats.merge(targs[["game_date", "game_pk", "yrfi"]], on=["game_date", "game_pk"], how="inner")
    merged = merged.dropna(subset=["game_date", "game_pk", "yrfi"]).copy()
    merged = merged.loc[merged["game_date"].dt.year == int(season)].copy()

    feature_cols = _select_feature_columns(merged.drop(columns=["yrfi"]))
    if not feature_cols:
        raise ValueError("No numeric feature columns found after exclusions")

    train_df, val_df = _time_split(merged)
    X_train = train_df[feature_cols]
    y_train = train_df["yrfi"].astype(int).to_numpy()
    X_val = val_df[feature_cols]
    y_val = val_df["yrfi"].astype(int).to_numpy()
    ...
    model_path = model_dir / f"yrfi_model_{season}.pkl"
    calibrator_path = model_dir / f"yrfi_calibrator_{season}.pkl"
    eval_path = Path("data/outputs") / f"yrfi_eval_{season}.csv"
    eval_path.parent.mkdir(parents=True, exist_ok=True)
    metadata_path = model_dir / f"yrfi_metadata_{season}.json"
    ...
    eval_df["p_yrfi_raw"] = p_raw
    eval_df["p_yrfi_cal"] = p_cal
    ...
    log.info("YRFI metrics: roc_auc=%.4f log_loss=%.4f brier=%.4f", auc, ll, brier)
...
def train_yrfi_from_paths(
    ...
    *,
    ...
    """Load features/targets from parquet paths and train YRFI model."""
