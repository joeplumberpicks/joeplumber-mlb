...
def infer_pitcher_ks_from_paths(
    season: int,
    features_path: Path,
    model_dir: Path,
    output_path: Path,
    *,
    ...
    preds = _predict(art["model"], art.get("model_type", "unknown"), features[feature_columns])
    ...
    log.info("wrote_pitcher_ks_preds season=%s rows=%d path=%s", season, len(out), output_path)
    return out
