...
ID_EXCLUDE = {"game_date", "game_pk", "pitcher_id", "team", "opponent_team", "starter_side"}
TARGET_CANDIDATES = ['outs']
...
def _resolve_target_col(targets_df: pd.DataFrame) -> str:
    for c in TARGET_CANDIDATES:
        if c in targets_df.columns:
            return c
    raise ValueError(f"targets missing OUTS target column; expected one of {TARGET_CANDIDATES}")
...
def run_smoke_test(logger: logging.Logger | None = None) -> None:
    ...
    y = 3.0 + 0.8 * np.nan_to_num(X[:, 0], nan=0.0) + rng.normal(0, 0.2, 40)
    preds = NumpyRidgeRegression(alpha=1.0).fit(X, y).predict(X)
    assert np.isfinite(preds).all()
    log.info("Pitcher Outs smoke test passed")


def train_pitcher_outs_model(
    season: int,
    ...
    *,
    ...
    req_tgt = {"game_date", "game_pk", "pitcher_id", target_col}
    ...
    merged = feats.merge(targs[["game_date", "game_pk", "pitcher_id", target_col]], on=["game_date", "game_pk", "pitcher_id"], how="inner")
    merged = merged.dropna(subset=["game_date", "game_pk", "pitcher_id", target_col]).copy()
    ...
    eval_path = Path("data/outputs") / f"pitcher_outs_eval_{season}.csv"
    ...
    eval_df["outs_pred"] = pred_val
    eval_df["y_true"] = y_val
    eval_df.sort_values(["game_date", "game_pk", "pitcher_id"], kind="mergesort").to_csv(eval_path, index=False)
    ...
    log.info("Pitcher OUTS metrics: rmse=%.4f mae=%.4f r2=%.4f", rmse, mae, r2)
    log.info("Pitcher OUTS reliability deciles:\n%s", rel.to_string(index=False))
...
def train_pitcher_outs_from_paths(
    season: int,
    features_path: Path,
    targets_path: Path,
    model_dir: Path,
    *,
    ...
    return train_pitcher_outs_model(
        season=season,
