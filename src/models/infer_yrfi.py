...
def infer_yrfi_from_paths(
    ...
    *,
    ...
    """Run YRFI inference and write calibrated probabilities."""
    ...
    out["p_yrfi_cal"] = p_cal
    out = out.sort_values(["game_date", "game_pk"], kind="mergesort").reset_index(drop=True)

    output_path.parent.mkdir(parents=True, exist_ok=True)
    out.to_csv(output_path, index=False)
    log.info("wrote_yrfi_preds season=%s rows=%d path=%s", season, len(out), output_path)
    return out
