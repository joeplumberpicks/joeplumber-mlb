...
TARGET_CANDIDATES = ['er', 'earned_runs']
...
def _resolve_target_col(targets_df: pd.DataFrame) -> str:
    for c in TARGET_CANDIDATES:
        if c in targets_df.columns:
            return c
    raise ValueError(f"targets missing ER target column; expected one of {TARGET_CANDIDATES}")
...
def run_smoke_test(logger: logging.Logger | None = None) -> None:
    ...
    log.info("Pitcher ER smoke test passed")


def train_pitcher_er_model(
    season: int,
    ...
    *,
    ...
    eval_path = Path("data/outputs") / f"pitcher_er_eval_{season}.csv"
    ...
    log.info("Pitcher ER metrics: rmse=%.4f mae=%.4f r2=%.4f", rmse, mae, r2)
    log.info("Pitcher ER reliability deciles:\n%s", rel.to_string(index=False))
...
def train_pitcher_er_from_paths(
    season: int,
    features_path: Path,
    targets_path: Path,
    model_dir: Path,
