...
def run_smoke_test(logger: logging.Logger | None = None) -> None:
    ...
    log.info("Hitter HR smoke test passed")


def train_hitter_hr_model(
    season: int,
    ...
    *,
    ...
    log = logger or logging.getLogger(__name__)
    req_feat, req_tgt = {"game_date", "game_pk", "batter_id"}, {"game_date", "game_pk", "batter_id", "hr"}
    ...
    targs["hr"] = pd.to_numeric(targs["hr"], errors="coerce").astype("Int64")

    merged = feats.merge(targs[["game_date", "game_pk", "batter_id", "hr"]], on=["game_date", "game_pk", "batter_id"], how="inner")
    merged = merged.dropna(subset=["game_date", "game_pk", "batter_id", "hr"]).copy()
    merged = merged.loc[merged["game_date"].dt.year == int(season)].copy()

    feat_cols = _select_feature_columns(merged.drop(columns=["hr"]))
    if not feat_cols:
        raise ValueError("No numeric feature columns found after exclusions")

    train_df, val_df = _time_split(merged)
    model, model_type = _fit_base_model(train_df[feat_cols], train_df["hr"].astype(int).to_numpy())
    p_raw = _predict_raw(model, model_type, val_df[feat_cols])

    calibrator, cal_method = _fit_calibrator(val_df["hr"].astype(int).to_numpy(), p_raw)
    p_cal = _apply_calibrator(calibrator, p_raw)

    auc = _safe_auc(val_df["hr"].to_numpy(), p_cal)
    ll = _log_loss(val_df["hr"].to_numpy(), p_cal)
    br = _brier(val_df["hr"].to_numpy(), p_cal)
    dec = _calibration_deciles(val_df["hr"].to_numpy(), p_cal)

    model_dir.mkdir(parents=True, exist_ok=True)
    model_path = model_dir / "model.pkl"
    cal_path = model_dir / "calibrator.pkl"
    meta_path = model_dir / "metadata.json"
    eval_path = Path("data/outputs") / f"hitter_hr_eval_{season}.csv"
    ...
    eval_df["p_hr_raw"] = p_raw
    eval_df["p_hr_cal"] = p_cal
    eval_df["y_true"] = val_df["hr"].astype(int).to_numpy()
    ...
    log.info("Hitter HR metrics: roc_auc=%.4f log_loss=%.4f brier=%.4f", auc, ll, br)
    log.info("Hitter HR calibration deciles:\n%s", dec.to_string(index=False))
...
def train_hitter_hr_from_paths(
    season: int,
    features_path: Path,
    targets_path: Path,
    model_dir: Path,
