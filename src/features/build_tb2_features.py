...
def build_tb2_features(
    ...
    if "tb_roll15" in df.columns and "opp_hardhit_rate_allowed_roll15" in df.columns:
        df["tb2_contact_pressure_roll15"] = (
    ...
    if "barrel_rate_roll15" in df.columns and "opp_hr_allowed_roll15" in df.columns:
        df["tb2_power_combo_roll15"] = (
            pd.to_numeric(df["barrel_rate_roll15"], errors="coerce")
            * pd.to_numeric(df["opp_hr_allowed_roll15"], errors="coerce")
        )

    # Apply lineup weighting to hitter-side features
    for c in [
        "tb_roll15",
        "tb_roll30",
        "hardhit_rate_roll15",
