...
def main() -> None:
    ...
    if "weather_wind_out" in df.columns:
        score = score - 0.05 * pd.to_numeric(df["weather_wind_out"], errors="coerce").fillna(0.0)

    df["home_win_score_raw"] = score
    df["home_win_prob"] = 1.0 / (1.0 + pd.Series((-score).clip(-20, 20)).map(lambda x: __import__("math").exp(x)))
    df["away_win_prob"] = 1.0 - df["home_win_prob"]
    df["pick"] = df["home_win_prob"].ge(0.5).map({True: "HOME", False: "AWAY"})

    keep = [c for c in ["game_date", "away_team", "home_team", "home_win_prob", "away_win_prob", "pick"] if c in df.columns]
    board = df[keep].sort_values("home_win_prob", ascending=False).reset_index(drop=True)

    out_path = outputs_dir / f"moneyline_board_{season}_{date_str}.csv"
    board.to_csv(out_path, index=False)

    print(f"✅ Moneyline board built: {out_path}")
