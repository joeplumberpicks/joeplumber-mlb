...
def main() -> None:
    ...
    base_score = first_non_null_numeric(df, ["tb2_score_raw", "score_raw", "score"], default=0.0)
    ...
    if hard_hit_rate.isna().all():
        hard_hit_rate = (tb_per_pa * 0.52).clip(lower=0.28, upper=0.62)
    if barrel_rate.isna().all():
        barrel_rate = (hr_per_pa * 2.20).clip(lower=0.03, upper=0.18)
    ...
    lineup_bonus = np.where(lineup_spot <= 2, 1.05, 1.00)
    lineup_bonus = np.where(lineup_spot == 3, 1.08, lineup_bonus)
    lineup_bonus = np.where(lineup_spot == 4, 1.10, lineup_bonus)
    lineup_bonus = np.where(lineup_spot == 5, 1.04, lineup_bonus)
    lineup_bonus = np.where(lineup_spot >= 7, 0.90, lineup_bonus)
    lineup_bonus = np.where(lineup_spot >= 8, 0.82, lineup_bonus)
    lineup_bonus = np.where(lineup_spot >= 9, 0.74, lineup_bonus)
    ...
    df["p_tb2"] = logistic(-0.55 + (df["z_score"] * 0.92))
