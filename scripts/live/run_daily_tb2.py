...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run daily TB2 board.")
...
def main() -> None:
    ...
    park_1b_factor = first_non_null_numeric(df, ["park_1b_factor"], default=1.0)
    park_2b3b_factor = first_non_null_numeric(df, ["park_2b3b_factor"], default=1.0)
    park_hr_factor = first_non_null_numeric(df, ["park_hr_factor"], default=1.0)
    tb2_weather_delta = first_non_null_numeric(df, ["tb2_weather_delta"], default=0.0)
    ...
    park_1b_factor = fill_with_median(park_1b_factor, 1.0)
    park_2b3b_factor = fill_with_median(park_2b3b_factor, 1.0)
    park_hr_factor = fill_with_median(park_hr_factor, 1.0)
    tb2_weather_delta = fill_with_median(tb2_weather_delta, 0.0)

    avg_ev_norm = ((ev - 88.0) / 10.0).clip(-1.5, 1.5)
    avg_la_norm = ((la - 12.0) / 12.0).clip(-1.5, 1.5)
    ...
    contact_only_tb2 = (
        ...
        avg_ev_norm * 0.35 +
        avg_la_norm * 0.15 +
        opp_pitcher_hard_hit_rate * 0.20 +
        opp_pitcher_barrel_rate * 0.15 +
        opp_pitcher_hr9 * 0.12
    )
    ...
    lineup_bonus = np.select(
        ...
        [0.05, 0.10, 0.12, 0.08, -0.12],
        default=0.00,
    )

    score = contact_only_tb2 + tb2_weather_delta + lineup_bonus
    ...
    df["p_tb2"] = logistic(-0.72 + (df["z_score"] * 0.82)).clip(0.15, 0.78)
