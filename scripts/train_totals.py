...
from src.models.train_totals import run_smoke_test, train_totals_from_paths


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Totals (O/U) model from game features + targets.")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Model directory (default: data/models/totals_{season}/)",
    )
    parser.add_argument("--smoke-test", action="store_true", help="Run synthetic smoke test and exit")
...
def main() -> None:
    setup_logging()
    args = parse_args()
    logger = logging.getLogger("train_totals")
    ...
    model_dir = Path(args.model_out) if args.model_out else Path(f"data/models/totals_{args.season}")

    train_totals_from_paths(
        season=args.season,
