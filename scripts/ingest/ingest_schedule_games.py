#!/usr/bin/env python3
"""
Ingest normalized MLB schedule games.

Purpose
-------
Fetch MLB schedule data for a date range or full season and write normalized
schedule outputs to the data lake.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ingest normalized MLB schedule games.")
...
def main() -> None:
    ...
    log_section("scripts/ingest/ingest_schedule_games.py")
    ...
    latest_out = raw_live_dir / f"games_schedule_{args.season}.parquet"
    write_parquet(df, latest_out)

    if args.start_date and (args.end_date is None or args.end_date == args.start_date):
        dated_out = raw_live_dir / f"games_schedule_{args.season}_{args.start_date}.parquet"
        write_parquet(df, dated_out)
    elif args.start_date and args.end_date:
        dated_out = raw_live_dir / f"games_schedule_{args.season}_{args.start_date}_{args.end_date}.parquet"
        write_parquet(df, dated_out)
    else:
        season_out = processed_by_season_dir / f"games_schedule_{args.season}.parquet"
