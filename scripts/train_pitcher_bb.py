...
from src.models.train_pitcher_bb import run_smoke_test, train_pitcher_bb_from_paths


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train pitcher BB regression model.")
    ...
    p.add_argument("--model-out", type=str, default=None, help="default data/models/pitcher_bb_{season}/")
...
def main() -> None:
    setup_logging()
    args = parse_args()
    logger = logging.getLogger("train_pitcher_bb")
    ...
    model_out = Path(args.model_out) if args.model_out else Path(f"data/models/pitcher_bb_{args.season}")

    train_pitcher_bb_from_paths(
        season=args.season,
