...
from src.models.train_yrfi import run_smoke_test, train_yrfi_from_paths


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train YRFI model from game features + targets.")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Model directory (default: data/models/yrfi_{season}/)",
    )
    parser.add_argument(
        "--smoke-test",
        action="store_true",
...
def main() -> None:
    setup_logging()
    args = parse_args()
    logger = logging.getLogger("train_yrfi")
    ...
    model_dir = Path(args.model_out) if args.model_out else Path(f"data/models/yrfi_{args.season}")

    train_yrfi_from_paths(
        season=args.season,
